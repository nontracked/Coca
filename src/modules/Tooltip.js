...
class Tooltip {
  ...
  constructor(rootElement) {
    ...
    this.continentElement = this.rootElement.querySelectorAll(
      this.selectors.continent
    )
    this.tooltipElement = this.rootElement.querySelector(this.selectors.tooltip)
    this.tooltipTitleElement = this.rootElement.querySelector(
      this.selectors.title
    )
    this.tooltipDescElement = this.rootElement.querySelector(
      this.selectors.desc
    )
    this.bindEvents()
  }
  ...
  tooltipPosition(event) {
    const topPosition =
      event.target === this.continentElement[1] ||
      event.target === this.continentElement[5]
        ? 150
        : 170
